    ...
    function initGui() {
        ...
        // datGui.add(gui, 'constantRotate', false).name('Always Rotate').listen();
        // datGui.add(gui, 'translateX', -0.2, 0.2).name('Translate X').listen();
        // datGui.add(gui, 'translateY', -0.2, 0.2).name('Translate Y').listen();
        // datGui.add(gui, 'translateZ', -0.2, 0.2).name('Translate Z').listen();
        datGui.add(gui, 'rotateX', -5, 5).name('rotateX').listen();
        datGui.add(gui, 'rotateY', -5, 5).name('rotateY').listen();
        datGui.add(gui, 'rotateZ', -5, 5).name('rotateZ').listen();
        datGui.add(gui, 'scaleX', 0.0, 5.0).name('scaleX').listen();
        datGui.add(gui, 'scaleY', 0.0, 5.0).name('scaleY').listen();
        datGui.add(gui, 'scaleZ', 0.0, 5.0).name('scaleZ').listen();
        // datGui.add(gui, 'texture',['Cube','MCBlock']).name('texture').onChange(function(value) {
            // switch (value) {
            //     case 'Cube':
            //         facematerial = new THREE.MeshFaceMaterial(materialArr1);
            //         break;
            //     case 'MCBlock':
            //         facematerial=new THREE.MeshFaceMaterial(materialArr2);
            //         break;
            //     };
            //     initModel();
            //     console.log(1);
    ...
    function animate() {
        ...
        cube.rotation.x = gui.rotateX;
        cube.rotation.y = gui.rotateY;
        cube.rotation.z = gui.rotateZ;
        cube.scale.x = gui.scaleX;
        cube.scale.y = gui.scaleY;
        cube.scale.z = gui.scaleZ;
